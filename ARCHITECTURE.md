...
src/app/(app)/systems/[id]/ the merchant shell — _layout (header + rail/drawer), Home, seven stubs
...
supabase/migrations/       profiles and merchants with their policies, the signup trigger,
                           delete_current_user, current_merchant_ids, and rls_auto_enable — whose
                           event trigger hosted Supabase refuses to install
...
docs/                      conventions for what is built on the scaffold — structure, data layer,
                           tenancy, layout, typography, visual language, migrations. See CLAUDE.md §2
...
trigger that would auto-enable RLS (not installable on the hosted project — see below), and one
