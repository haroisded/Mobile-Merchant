...
src/themes.js              MD3 light/dark palettes + Merchant keys, roundness 0, the type scale
src/features/merchants/    schema, queries, SystemCard, CreateSystemModal, RemoveSystemDialog
src/features/profiles/     the profile read; ProfileScreen, rendered by the Account tab and profile.tsx
src/features/products/     schema, queries, list, form (form/: sections + fields), detail, archive dialog
src/features/categories/   queries, CategoryPicker, CategoryDialog, CategoriesSection (Products Setup)
src/features/tax-classes/  queries, TaxClassPicker with inline create and presets, TaxClassesSection
src/features/suppliers/    queries, SupplierPicker with inline create, SuppliersSection
src/components/            PageHeader, AdaptiveDialog, MenuSelect — each drawn by more than one screen
src/app/_layout.tsx        PaperProvider + QueryProvider + the two-state route guard
src/app/sign-in.tsx        provider buttons
src/app/(app)/_layout.tsx  signed-in group: tabs, systems/ and profile beside them, anchored on tabs
...
docs/                      conventions for what is built on the scaffold — structure, data layer,
                           tenancy, layout, typography, visual language, migrations, device
                           testing, testing workflow. See CLAUDE.md §2
...
.oxlintrc.json             oxlint config: rule list + the local plugin it loads, and the overrides
                           that turn no-design-literals on for src/** and off for src/themes.js
...
exactly that reason ([docs/tenancy.md §3](./docs/tenancy.md)).
...
`onError` — **Delete account** and **Sign Out** in `src/features/profiles/ProfileScreen.tsx`, and
...
`src/app/_layout.tsx`, Feather first with MaterialCommunityIcons as the fallback. Paper's own default goes through `react-native-vector-icons`, whose font
